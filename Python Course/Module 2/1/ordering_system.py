...
def calculate_subtotal(order):
    ...
    ### WRITE SOLUTION HERE
    #sub_total=0
   # prices_list = [price["price"] for price in order]
   #  for item_price in prices_list:
   #      sub_total += item_price
   # return round(sub_total,2)
   ### Iterates over order which are three dictionaries and then for each it gets the value that is assosciated with key price [expression for item in iterable if condition]
    return round(sum([price["price"] for price in order]),2)
...
def summarize_order(order):
    ...
    print_order(order)
    ### WRITE SOLUTION HERE
    amount_subtotal = calculate_subtotal(order)
    amount_tax = calculate_tax(amount_subtotal)
    amount_total = amount_subtotal + amount_tax
    ### Iterates over order which are three dictionaries and then for each it gets the value that is assosciated with key name [expression for item in iterable if condition]
    item_names = [items["name"] for items in order]
    return round(item_names, 2), round(amount_total,2)
